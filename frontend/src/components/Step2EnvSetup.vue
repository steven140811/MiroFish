              ...
              <div v-if="useCustomRounds" class="rounds-content custom" key="custom">
                <div class="slider-display">
                  ...
                  <div class="slider-meta-info">
                    <span>{{ $t('step2.estimatedDuration', estimatedDurationParams(customMaxRounds)) }}</span>
                  </div>
                </div>
                  ...
                  <div class="auto-content">
                    <div class="auto-meta-row">
                      <span class="duration-badge">
                        ...
                        {{ $t('step2.estimatedDurationFull', estimatedDurationParams(autoGeneratedRounds)) }}
                      </span>
                    </div>
                    <div class="auto-desc">
...
const simulatedHoursForRounds = (rounds) => {
  const minutesPerRound = simulationConfig.value?.time_config?.minutes_per_round || 60
  return Math.round((rounds * minutesPerRound / 60) * 10) / 10
}

const estimatedWallMinutesForRounds = (rounds) => {
  const agentCount = profiles.value.length || 100
  const timeConfig = simulationConfig.value?.time_config || {}
  const activeMin = Number(timeConfig.agents_per_hour_min || 0)
  const activeMax = Number(timeConfig.agents_per_hour_max || 0)
  const activeAgents = activeMin && activeMax
    ? (activeMin + activeMax) / 2
    : Math.min(agentCount, 20)
  const secondsPerRound = 8 + activeAgents * 0.6 + Math.min(agentCount, 100) * 0.03
  return Math.max(1, Math.round(rounds * secondsPerRound / 60))
}

const estimatedDurationParams = (rounds) => ({
  minutes: estimatedWallMinutesForRounds(rounds),
  simHours: simulatedHoursForRounds(rounds)
})
